...
from src.app.models.user import PendingUser
...
async def register_user_service(new_user: RegisterUserSchema, db: AsyncSession) -> PendingUser:
  ...
  try:
    ...
    user_db = PendingUser(**user_dict) # pass new user with hashed password to the model
    db.add(user_db)
    await db.commit()
